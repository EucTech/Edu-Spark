...
function FormField({
  ...
  value: string; onChange: (v: string) => void; required?: boolean; icon: React.ElementType;
}) {
  return (
...
export function AddEditCourseModal({ open, onClose, course, onSave }: AddEditCourseModalProps) {
  ...
  useEffect(() => {
    if (course) {
      setForm({ title: course.title, description: course.description, grade_group: course.grade_group });
    } else {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md p-0 overflow-hidden rounded-2xl border border-[#e4e6f0]">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 grid gap-4">
          ...
          <FormField id="c_grade_group" label="Grade Group" placeholder="e.g. P1, P2, P3"
            value={form.grade_group} onChange={(v) => setForm({ ...form, grade_group: v })} required icon={LuLayers} />
          ...
          <div className="flex items-center justify-end gap-3 pt-2 border-t border-[#f0f1f7]">
            ...
            <Button type="submit" className="bg-[#3749a9] hover:bg-[#2d3b8e] text-white">
